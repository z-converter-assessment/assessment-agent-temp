...
- 이 계약은 산문이 아니라 기계검증으로 강제된다: `schema/wire.schema.json`(정본) + 두 바이너리의 `emit` dry-run 출력을 CI가 이 스키마로 검증(`scripts/check-contract.sh`). 필드/타입/null·os_family 조건부(saturation은 Windows 전용 등)를 어기면 릴리즈가 막힌다. 스키마·직렬화를 바꾸면 스키마 정본도 같이 고친다.
