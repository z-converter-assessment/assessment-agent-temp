...
## testbed (testbed-environment-handoff.md 참조)

Linux: st-lvm/st-raid/st-btrfs/st-mpath/st-nested(storage-testbed-net), centos6(rescue).
Windows: dp-win2016(구세대 viostor 생존, 부하하네스 부착). WinRM Administrator/<redacted> (ntlm).

## 사용 예

```
ssh -i ~/.ssh/agent-fleet-key.pem ubuntu@10.50.60.126 'sudo python3 -' < scripts/accept/ts_integrity_linux.py
python3 scripts/accept/winrm_run.py 10.50.50.174 ntlm < some.ps1
```
