...
> 발송 조건: 데이터모델(wire v2) 락 확정 시 엔진 docs/temp 로 발송. 그 전엔 PENDING(agent temp 보관).
...
E축 완전체(mdraid/btrfs/ext4/ioerr, net errors/dropped/tcp.retransmits/conntrack, memory.hardware_corrupted,
cpu.mce)를 넣은 이유와 예상 소비를 명확히 한다 — 이건 사이징 숫자로 가지 않는다.
