...
산출물은 `dist/assessment-agent-linux-x86_64`. build-linux.sh가 컨테이너 안에서 `make clean` -> `vendor-build`(musl) -> `release`를 돌리고, verify가 완전 static(PT_INTERP 없음, GLIBC 심볼 0)을 확인한다. 빌드 이미지는 `BUILD_IMAGE`(기본 `alpine:3.19`)로 바꿀 수 있다.
...
`v*` 태그를 push하면 2종을 빌드해 해당 태그의 GitHub Release에 게시한다. 릴리즈는 최신 단일 태그(현재 v1.0.5) 하나로 유지하고, 재릴리즈는 그 태그를 덮어쓴다.

```bash
git tag -f v1.0.5 && git push -f origin v1.0.5
```
