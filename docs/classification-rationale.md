...
- memory.usage state=available (MemAvailable / Windows Available): used% 는 회수 가능한 캐시까지 사용으로
  쳐 압박을 과대평가한다. available 은 회수 반영 실가용이라 이걸 발행한다. 예상 소비: 압박 = 1 -
  available/limit (used% 아님). 구커널(centos6 2.6.32)은 MemAvailable 부재라 MemFree+Buffers+Cached 계산 폴백.
- network.link.speed: 처리량만으론 "한계 대비 %"를 못 낸다. 링크속도 분모를 함께 실어 엔진이 util =
  throughput/link_speed 를 산출한다. 예상 소비: 네트워크 이용률. 단 virtio 는 링크속도 null -> 이용률 미산출(수용).
- disk.io_time (%util): await(포화, S축)만으론 장치 이용률(busy 시간 비율)을 못 낸다. io_time 으로 USE 의
  U축(이용률)을 채운다. 예상 소비: 디스크 이용률 = busy 시간분율. (await/queue 는 S축, 별개.)
...
block_devices(parent-by-id) + lvm_vgs(free_bytes): major/minor 조인은 dm/LVM 에서 끊긴다. parent-by-id 로
