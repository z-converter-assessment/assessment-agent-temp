...
재설정은 `agent.env` 수정 후 `sc stop`/`sc start`. 실행 로그를 콘솔에서 직접 보려면 `assessment-agent-windows-x86.exe run`으로 포그라운드 실행한다. 실행 시 OS 세대를 감지해 최신 OS에서는 64비트 카운터, IPv6 게이트웨이, 프로세스 경로 등 풍부한 데이터를, NT5.2(2003/XP)에서는 그 OS가 지원하는 범위의 데이터를 수집한다.
