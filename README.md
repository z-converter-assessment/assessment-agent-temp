...
서비스 등록(install)에 필요한 권한과 서비스가 실행되는 계정 권한은 별개다. 빌드별로 다르다.

| 빌드 | install 권한 | 런타임 실행 계정 | 서비스 방식 |
|------|--------------|------------------|-------------|
| linux-x86_64 | 일반 유저 (root 불필요) | 설치한 일반 유저 | systemd user (`systemctl --user`) |
| linux-x86_64-glibc2.12 | root 1회 (`sudo`) | 비특권 `assessment-agent` 유저 | SysV init (`/etc/init.d`) |
| win2016 / win2008r2 / win2003 | Administrator (관리자 명령프롬프트) | LocalSystem | Windows 서비스 (SCM) |

수집 커버리지 관점: 윈도우 3종은 LocalSystem(최고 권한)으로 돌아 특권이 필요한 필드까지 채운다. 리눅스는
비특권 계정으로 돌리는 설계라, 다른 유저 소유 프로세스의 실행 경로(`/proc/<pid>/exe`)·cmdline 등 root가
있어야 읽히는 일부 필드는 접근 불가로 `null`이 될 수 있다(프로세스는 정상 동작, 해당 필드만 비어서 전송).
전체 필드 커버리지가 필요하면 리눅스도 root/특권 계정으로 실행하면 된다.

## (1) assessment-agent-linux-x86_64 — Ubuntu 18.04+, RHEL/CentOS 7-9, Amazon Linux 2/2023, SUSE 12/15

설치 (설치할 유저로 실행, root 불필요):

```bash
chmod +x assessment-agent-linux-x86_64
./assessment-agent-linux-x86_64 install
```

무인 설치(플레이스홀더를 실제 값으로 바꿔 한 줄 실행):

```bash
RABBITMQ_HOST='<브로커주소>' RABBITMQ_PASS='<브로커비밀번호>' RABBITMQ_WORKER_PASS='<worker비밀번호>' ./assessment-agent-linux-x86_64 install < /dev/null
```

`install`이 브로커 주소/worker 허용 호스트/비밀번호를 프롬프트로 받는다. 바이너리는 `~/.local/bin`,
설정은 `~/.config/assessment-agent/agent.env`(비밀은 `agent.env.local`, 0600)에 놓이고 `systemctl --user`로
상시 구동된다. lingering을 자동 활성화해 로그인 없이 부팅 시 시작한다(권한 없으면 다음 로그인부터).

운영:

```bash
systemctl --user status assessment-agent        # 상태
journalctl --user -u assessment-agent -f         # 로그
# 재설정: ~/.config/assessment-agent/agent.env 수정 후
systemctl --user restart assessment-agent
./assessment-agent-linux-x86_64 uninstall        # 제거
```

VM 이미지로 복제 배포하는 경우 골든 이미지 봉인 직전에 `./assessment-agent-linux-x86_64 prep-image`를
실행해 머신 식별자를 초기화한다(클론마다 고유 ID 확보).

## (2) assessment-agent-linux-x86_64-glibc2.12 — RHEL/CentOS 6

systemd가 없는 세대라 `/etc/init.d` 서비스 등록에 root가 1회 필요하다(런타임은 비특권 assessment-agent 유저).
