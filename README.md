...
|------|---------|--------------|----------|
| assessment-agent-linux-x86_64 | 전 x86_64 Linux, kernel >= 2.6.32 | musl fully-static (glibc/배포판 무관) | Alpine musl |
...
따로 설치할 필요가 없다. Linux 바이너리는 musl 완전 static이라 glibc 버전이나 배포판과
무관하게 커널 2.6.32 이상 x86_64 리눅스면 동작한다. Windows 바이너리는
