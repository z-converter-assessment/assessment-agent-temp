...
따로 설치할 필요가 없다 — 파일 하나가 곧 에이전트다. Linux 바이너리는 musl 완전 static이라 glibc 버전과
...
Windows에서도 그대로 동작한다. 실행 시 OS 세대를 감지해 최신 OS에서는 64비트 카운터·IPv6 게이트웨이·프로세스
경로 등 풍부한 데이터를, 구세대에서는 그 OS가 지원하는 범위의 데이터를 수집한다. 2종 모두 `install`
서브커맨드가 설치부터 설정 프롬프트, 서비스 등록까지 처리한다. 아래에서 대상 OS 항목 하나만 그대로 따라가면
되고, 각 항목은 자기완결적이다.

구분법: OS 계열(Linux / Windows)만 보면 된다. Linux는 배포판/glibc 구분이 필요없고, Windows는 NT 세대
구분이 필요없다 — 각 계열당 바이너리가 하나다. 설치 시 브로커 주소(RABBITMQ_HOST), worker 다운로드 허용
호스트(WORKER_DOWNLOAD_ALLOWED_HOSTS), 비밀번호를 대화형으로 물어보고 나머지 값은 기본값을 쓴다.
