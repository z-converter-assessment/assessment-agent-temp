...
static int wait_confirm(amqp_connection_state_t conn, uint64_t expected_tag)
{
	...
	for (;;) {
		...
		if (frame.frame_type != AMQP_FRAME_METHOD || frame.channel != 1)
			continue;
		/* confirm 은 delivery-tag(발행 seqno)로 메시지를 지목 — expected_tag 의 ack(또는 multiple 누적
		 * ack)만 성공으로 본다. 이전 메시지의 지각 ack 를 이번 confirm 으로 오인해 유실을 놓치지 않도록. */
...
/* oneshot 발행과 worker 상시 연결이 공유하는 연결 셋업 — TLS verify/cacert/cert 와
 * FD_CLOEXEC 가 두 경로에서 갈리지 않게 한다. who 는 로그 접두사. */
                         ...
                         size_t      body_len)
{
	...
	/* confirm seqno 는 basic.publish 성공분만 증가한다(실패분은 seqno 를 소비하지 않음). */
	uint64_t tag = ++c->next_publish_seqno;
	return wait_confirm(c->conn, tag);
}

/* task 큐(agent.tasks.{agent_id})는 engine 이 첫 task 때 lazy 생성 — 그전 basic.get 은 404 로
 * 채널을 닫는다. 에러가 아니라 "큐 아직 없음(대기)". 이 채널 예외만 가려낸다(다른 예외는 에러). */
...
int publish_conn_recover_channel(publish_conn_t *c)
{
	...
	/* 채널 예외(큐 404 등) 후 채널 1 을 재오픈 — 연결은 살아있어 전체 reconnect 없이 폴링 지속.
	 * confirm 모드라 confirm.select 재적용. 실패 시 호출자가 전체 reconnect 로 폴백. */
	...
	if (amqp_get_rpc_reply(c->conn).reply_type != AMQP_RESPONSE_NORMAL)
		return -1;
	/* confirm.select 재적용으로 broker 의 confirm seqno 가 1 로 리셋 — 클라이언트 카운터도
	 * 맞춰 리셋해야 tag 매칭이 어긋나지 않는다. */
